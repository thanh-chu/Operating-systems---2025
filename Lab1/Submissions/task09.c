...
int main(int argc, char** argv) {
    ...
    if (argc > 1)
        nThreads = atoi(argv[1]); //change string to int. e.g when I run program: ./bankaccount 16 => change '16' to int => nThreads = 16
    children = malloc( nThreads * sizeof(pthread_t) ); //dynamic memory allocation on heap.
